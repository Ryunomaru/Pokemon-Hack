...
Vedio le differenti possibilità durante la modifiche delle mappe.
...
Ctrl + Tasto destro del mouse		=> Permette di scegliere e posizionare più blocchi alla volta.
...
Qui ci sono due parti. La prima, tutti i parametri dei linguaggi sono sotto "spiele"- listing, perchè "inkSprache" è settato a 1.
Also we have the pointer to the at point "vor" described Bytes.
Dato che non tutti i giochi usano la prima parte, ce n'è una seconda universale.
Dopo la seconda parte, il pointer sarà trovato alla posizione impostata nella variabile "position".
...
Main.ini
--------
Questo è l'ordine di come le mappe vengono listate.
...
La parte "[0.0]" si riferisce alla posizione dell'header mappa. Se alterato, la mappa potrebbe non funzionare più!
Gli offset non sono più usati in questa versione ma possono tranquillamente rimanere.
...
Tilesets.ini
------------
Questo file è stato rinnovato dalla versione 1.30.
Volendo puoi cambiare il numero di blocchi associato a ciascun tileset (il primo tileset è il numero 0).
Se il tileset è nella parte superiore avrà automaticamente 200 blocchi.
...
Sprachen.ini
------------
Qui c'è la lista di tutte le lingue disponibili.
...
Finché non hai fatto ciò, la lingua non è disponibile...
La lingua può essere cambiata tramite "Impostazioni"->"Lingua".

Al primo avvio di AdvanceMap è possibile scegliere la lingua che preferisci.
...
Il valore all'offset specificato dovrebbe essere un pointer, ma non lo è. (-> dati errati)
 
AdvanceMapError(6): Try to read more than 1023 Bytes a time! Please contakt luhopoke@gmail.com
AdvanceMap, tramite una funzione speciale, ha tentato di leggere più di 1023 byte. (-> bug in AdvanceMap)
 
AdvanceMapError(7): Try to save more than 1023 Bytes a time! Please contakt luhopoke@gmail.com
AdvanceMap, tramite una funzione speciale, ha tentato di scrivere più di 1023 byte. (-> bug in AdvanceMap)
...
AdvanceMap non può leggere una ROM compressa in un archivio zip/rar in quanto va prima decompressa. (-> attenzione ^^)
...
La ROM aperta non corrisponde ad un gioco supportato. Solo le ROM Pokémon della generazione Advance possono essere modificate con AdvanceMap.
